...
export async function GET() {
  try {
    ...
    const BUSINESS_NAME = process.env.GOOGLE_PLACE_NAME || 'Chauffeurio Transfers Valencia';
      ...
      if (!GOOGLE_MAPS_API_KEY) {
        return NextResponse.json(
          { 
            ...
            message: 'Please set NEXT_PUBLIC_GOOGLE_MAPS_API_KEY or GOOGLE_PLACES_API_KEY in your environment variables'
          },
          { status: 400 }
        );
      }
    ...
    // To get more reviews, you would need to use Google Business Profile API
    // (if you own the business) or a third-party service
    const sortedReviews = reviews
      .sort((a, b) => b.time - a.time)
      .slice(0, 10); // Will only get up to 5 from Google Places API
