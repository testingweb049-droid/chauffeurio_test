...
function ContinueButton({title, loading , type, step}:{title:string, loading?:boolean, type:'submit'|'button', step:number}) {
  ...
  return (
    <button type={type} onClick={()=>{changeStep(true,step); }} className={`flex items-center justify-center gap-2 w-full p-2 rounded-lg cursor-pointer font-semibold ${formLoading || loading ? 'bg-blue-500 text-white' : 'bg-primary text-white '} `}>
