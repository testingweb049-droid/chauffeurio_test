      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className='flex flex-col gap-5 w-full max-lg:hidden'>
                <div className='w-full rounded-xl bg-black flex flex-col gap-8 p-5'>
                  <Image src="/Container (2).png" alt='card chip' height={200} width={200} className='w-12' />
                ...
                <Image height={200} width={200} src="/Container (2).png" alt='secure 1' className='' />
                <Image height={200} width={200} src="/Container (2).png" alt='secure 2' className='' />
                </div> */}
            </div>
...
function MyPaymentForm({price}:{price:string}) {
  ...
  useEffect(() => {
    const fetchClientSecret = async () => {
      try {
        const response = await fetch("https://chauffeurio.com/api/create-payment-intent", {
          method: "POST",
          headers: {
